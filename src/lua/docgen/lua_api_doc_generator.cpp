...
void LuaBindingScanner::parseLuaReg(const std::string &content, const std::filesystem::path &filePath, LuaScanResult &result) const {
	// luaL_Reg is not used by this codebase, but kept in case there is some legacy code
...
bool LuaApiDocGenerator::exportMarkdown() const {
	...
	output << "# Lua API\n\n";
	output << "This file is auto-generated from Crystal Server's C++ Lua bindings. Do not edit it manually.\n\n";
	...
	output << "For manual setup, add `" << docsDirectoryPath << "` or `" << docsFilePath("lua_api.d.lua")
		   << "` to the Lua Language Server workspace library. Crystal Server updates these files during startup when `generateLuaApiDocs` is enabled in `config.lua`.\n\n";
