local timers = {
	[1] = { position = Position(32616, 32529, 13), boss = "mazzinor", toPosition = Position(32720, 32770, 10) },
	[2] = { position = Position(32464, 32654, 12), boss = "lokathmor", toPosition = Position(32720, 32746, 10) },
	[3] = { position = Position(32662, 32713, 13), boss = "ghulosh", toPosition = Position(32746, 32770, 10) },
	[4] = { position = Position(32660, 32736, 12), boss = "gorzindel", toPosition = Position(32746, 32746, 10) },
...
function movements_library_timers.onStepIn(creature, item, position, fromPosition)
	...
	for _, k in pairs(timers) do
		if position == k.position then
			if not k.boss or player:getBossCooldown(k.boss) <= os.time() then
