local mType = Game.createMonsterType("Pirate Quartermaster")
local monster = {}

monster.description = "a pirate quartermaster"
monster.experience = 95
monster.outfit = {
	lookType = 151,
	lookHead = 114,
	lookBody = 19,
	lookLegs = 23,
	lookFeet = 40,
	lookAddons = 2,
	lookMount = 0,
}

monster.raceId = 2779
monster.Bestiary = {
	class = "Human",
	race = BESTY_RACE_HUMAN,
	toKill = 1000,
	FirstUnlock = 50,
	SecondUnlock = 500,
	CharmsPoints = 15,
	Stars = 1,
	Occurrence = 0,
	Locations = "Targuna (Aragonia).",
}

monster.events = {
	"TargunaPirateDeath",
}

monster.health = 80
monster.maxHealth = 80
monster.race = "blood"
monster.corpse = 53103 -- corpse Targuna (appearances.dat)
monster.speed = 85
monster.manaCost = 0

monster.changeTarget = {
	interval = 4000,
	chance = 10,
}

monster.strategiesTarget = {
	nearest = 100,
}

monster.flags = {
	summonable = false,
	attackable = true,
	hostile = true,
	convinceable = false,
	pushable = false,
	rewardBoss = false,
	illusionable = true,
	canPushItems = true,
	canPushCreatures = false,
	staticAttackChance = 90,
	targetDistance = 1,
	runHealth = 10,
	healthHidden = false,
	isBlockable = false,
	canWalkOnEnergy = false,
	canWalkOnFire = false,
	canWalkOnPoison = false,
}

monster.light = {
	level = 0,
	color = 0,
}

monster.voices = {
	interval = 5000,
	chance = 10,
	{ text = "All hands on deck!", yell = false },
}

monster.loot = {
	{ name = "mana potion", chance = 30000 },
	{ name = "gold coin", chance = 50000, maxCount = 18 },
	{ name = "gold tooth", chance = 9000 },
	{ id = 5792, chance = 700 }, -- die
	{ name = "pirate knee breeches", chance = 700 },
	{ name = "pirate bag", chance = 700 },
	{ name = "eye patch", chance = 150 },
	{ name = "hook", chance = 150 },
	{ name = "peg leg", chance = 150 },
}

monster.attacks = {
	{ name = "melee", interval = 2000, chance = 100, minDamage = 0, maxDamage = -30 },
	{ name = "combat", interval = 2000, chance = 100, type = COMBAT_ICEDAMAGE, minDamage = 0, maxDamage = -20, range = 1, effect = CONST_ME_ICEATTACK, target = true },
}

monster.defenses = {
	defense = 16,
	armor = 3,
	mitigation = 0.40,
}

monster.elements = {
	{ type = COMBAT_PHYSICALDAMAGE, percent = 0 },
	{ type = COMBAT_ENERGYDAMAGE, percent = 0 },
	{ type = COMBAT_EARTHDAMAGE, percent = 0 },
	{ type = COMBAT_FIREDAMAGE, percent = 0 },
	{ type = COMBAT_LIFEDRAIN, percent = 0 },
	{ type = COMBAT_MANADRAIN, percent = 0 },
	{ type = COMBAT_DROWNDAMAGE, percent = 0 },
	{ type = COMBAT_ICEDAMAGE, percent = 0 },
	{ type = COMBAT_HOLYDAMAGE, percent = 0 },
	{ type = COMBAT_DEATHDAMAGE, percent = 0 },
}

monster.immunities = {
	{ type = "paralyze", condition = false },
	{ type = "outfit", condition = false },
	{ type = "invisible", condition = true },
	{ type = "bleed", condition = false },
}

mType:register(monster)
