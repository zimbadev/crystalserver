...
function serverInfo.onSay(player, words, param)
	...
	if useStages then
		...
		text = text .. "\nExp Stages rate: " ..
			getRateFromTable(experienceStages, player:getLevel(),
				expstagesrate) .. "x"

		for _, skillData in ipairs(skills) do
			text = text .. "\n" .. skillData.name .. " rate: " ..
				getRateFromTable(skillsStages,
					player:getSkillLevel(skillData.skill),
					configRateSkill) .. "x"
		end
		...
		text = text .. "\nMagic Stages rate: " ..
			getRateFromTable(magicLevelStages, player:getBaseMagicLevel(),
				configManager.getNumber(
					configKeys.RATE_MAGIC)) .. "x"
	else
		text = text .. "\nExp rate: " ..
			configManager.getNumber(configKeys.RATE_EXPERIENCE) .. "x" ..
			"\nSkill rate: " ..
			configManager.getNumber(configKeys.RATE_SKILL) .. "x" ..
			"\nMagic rate: " ..
			configManager.getNumber(configKeys.RATE_MAGIC) .. "x"
	end

	text = text .. "\nLoot rate: " ..
		configManager.getNumber(configKeys.RATE_LOOT) .. "x" ..
		"\nSpawns rate: " ..
		configManager.getNumber(configKeys.RATE_SPAWN) .. "x" ..
		"\nBestiary rate: " ..
	...
	text = text .. "\n\nAnimus Mastery: \n" .. "\nMax Monster XP: " ..
		configManager.getFloat(
			configKeys.ANIMUS_MASTERY_MAX_MONSTER_XP_MULTIPLIER) .. "x" ..
		"\nMonster XP: " ..
		configManager.getFloat(
			configKeys.ANIMUS_MASTERY_MONSTER_XP_MULTIPLIER) .. "x" ..
		"\nMonsters XP: " ..
		configManager.getFloat(
			configKeys.ANIMUS_MASTERY_MONSTERS_XP_MULTIPLIER) .. "x" ..
		"\nMonsters to Increase: " ..
		configManager.getNumber(
			configKeys.ANIMUS_MASTERY_MONSTERS_TO_INCREASE_XP_MULTIPLIER)
	...
	if staminaTrainer or staminaPz then
		...
		if staminaTrainer then
			text = text .. "\nTrainer enabled: Yes" .. "\nTrainer Delay: " ..
				configManager.getNumber(
					configKeys.STAMINA_TRAINER_DELAY) .. " min" ..
				"\nTrainer Gain: " ..
	...
	local loseHouseText =
		configManager.getNumber(configKeys.HOUSE_LOSE_AFTER_INACTIVITY) > 0 and
		configManager.getNumber(configKeys.HOUSE_LOSE_AFTER_INACTIVITY) ..
		" days" or "never"
	...
	text = text .. "\n\nHouse Info: \n" .. "\nLevel to buy: " ..
		...
		"\nRent rate: " ..
		configManager.getFloat(configKeys.HOUSE_RENT_RATE) .. "x"
	...
	text = text .. "\n\nVIP System: \n" .. "\nVIP Bonus Exp: " ..
		...
		(configManager.getBoolean(configKeys.VIP_AUTOLOOT_VIP_ONLY) and
			"Yes" or "No") .. "\nVIP Stay Online: " ..
		(configManager.getBoolean(configKeys.VIP_STAY_ONLINE) and "Yes" or
			"No") .. "\nVIP Familiar Time Cooldown Reduction: " ..
		configManager.getNumber(
			configKeys.VIP_FAMILIAR_TIME_COOLDOWN_REDUCTION) .. " min" ..
		"\nVIP Keep House: " ..
		(configManager.getBoolean(configKeys.VIP_KEEP_HOUSE) and "Yes" or
			"No")
