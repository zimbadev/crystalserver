...
function serverInfo.onSay(player, words, param)
	...
	if useStages then
		...
		for _, skillData in ipairs(skills) do
			...
			"\n" ..
			skillData.name ..
			" rate: " .. getRateFromTable(skillsStages, player:getSkillLevel(skillData.skill), configRateSkill) .. "x"
		end
	...
	local loseHouseText = configManager.getNumber(configKeys.HOUSE_LOSE_AFTER_INACTIVITY) > 0 and
	configManager.getNumber(configKeys.HOUSE_LOSE_AFTER_INACTIVITY) .. " days" or "never"
