...
function trainingRoms.onUse(player, item, fromPosition, target, toPosition, monster, isHotkey)
    ...
    if item.actionid == ACTION_IDS.NORTH then
        otherSidePosition = currentCharPosition.y < fromPosition.y and {x = fromPosition.x, y = fromPosition.y + 1, z = fromPosition.z}
                                                                         or {x = fromPosition.x, y = fromPosition.y - 1, z = fromPosition.z, stackpos = 253}
    elseif item.actionid == ACTION_IDS.SOUTH then
        otherSidePosition = currentCharPosition.y > fromPosition.y and {x = fromPosition.x, y = fromPosition.y - 1, z = fromPosition.z}
                                                                         or {x = fromPosition.x, y = fromPosition.y + 1, z = fromPosition.z, stackpos = 253}
    elseif item.actionid == ACTION_IDS.EAST then
        otherSidePosition = currentCharPosition.x > fromPosition.x and {x = fromPosition.x - 1, y = fromPosition.y, z = fromPosition.z}
                                                                        or {x = fromPosition.x + 1, y = fromPosition.y, z = fromPosition.z, stackpos = 253}
